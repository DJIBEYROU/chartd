        ...
        for idx, epoch in enumerate(epochs):
            for region in regions:
                ...
                if not isinstance(region_data, dict):
                    continue
                entry = {"date": datetime.fromtimestamp(epoch).strftime("%Y-%m-%d %H:%M:%S"), "region": region}
...
def categorize_energy_sources():
    return {
        "renewable": ["hydropower", "geothermal", "bioenergy", "solar", "wind", "pumping_up", "pumping_down", "battery_charge", "battery_generate"],
        "non_renewable": ["nuclear", "thermal_lng", "thermal_coal", "thermal_oil", "thermal_others", "others"],
    }
...
def aggregate_by_month(df):
    ...
    if len(unique_months) > 1:
        days_per_month = df.groupby(["month", "region"])["date"].apply(lambda x: len(x.dt.date.unique())).reset_index(name="day_count")
...
def get_energy_data():
    try:
        ...
        return jsonify({"type": {"daily": df_agg.to_json(orient="records", date_format="iso")}, "categorized": {"monthly": monthly_data.to_json(orient="records", date_format="iso")}})
    except Exception as e:
